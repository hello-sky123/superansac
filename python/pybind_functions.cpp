...
void initializeNeighborhood(
    ...
    const superansac::neighborhood::NeighborhoodType
    ...
    const superansac::RANSACSettings& kSettings_)  // The RANSAC settings
{
  ...
  if (kNeighborhoodType_ == superansac::neighborhood::NeighborhoodType::Grid) {
    ...
    for (size_t i = 0; i < _DimensionNumber; i++) {
      ...
      if (kCellSizes[i] < 1.0)
        throw std::invalid_argument(
            "The cell size is too small (< 1px). Try setting a smaller neighborhood size (in grid "
            "it acts as the number of cells along an axis).");
    }

    gridNeighborhoodGraph->setCellSizes(kCellSizes,    // The sizes of the cells in each dimension
    ...
    flannNeighborhoodGraph->setNearestNeighborNumber(
        kSettings_.neighborhoodSettings.nearestNeighborNumber);
...
void initializeLocalOptimizer(
    ...
    const superansac::neighborhood::NeighborhoodType
        kNeighborhoodType_,  // The type of the neighborhood
    const superansac::local_optimization::LocalOptimizationType
        kLocalOptimizationType_,  // The type of the neighborhood
    const std::vector<double>&
        kImageSizes_,  // Image sizes (height source, width source, height destination, width destination)
    const superansac::RANSACSettings& kSettings_,               // The RANSAC settings
    const superansac::LocalOptimizationSettings& kLOSettings_,  // The RANSAC settings
    const superansac::models::Types& kModelType_, const bool kFinalOptimization_ = false) {
  ...
  if (kLocalOptimizationType_ == superansac::local_optimization::LocalOptimizationType::GCRANSAC) {
    ...
    auto gcransacLocalOptimizer =
        dynamic_cast<superansac::local_optimization::GraphCutRANSACOptimizer*>(
            localOptimizer_.get());
  ...
  } else if (kLocalOptimizationType_ ==
             superansac::local_optimization::LocalOptimizationType::IRLS) {
    // Set the neighborhood graph to the local optimizer
    auto irlsLocalOptimizer =
        dynamic_cast<superansac::local_optimization::IRLSOptimizer*>(localOptimizer_.get());
    irlsLocalOptimizer->setMaxIterations(kLOSettings_.maxIterations);
    if (kFinalOptimization_ || kModelType_ == superansac::models::Types::Homography ||
    ...
    // Set the neighborhood graph to the local optimizer
    auto lsqLocalOptimizer =
        dynamic_cast<superansac::local_optimization::LeastSquaresOptimizer*>(localOptimizer_.get());
    if (kFinalOptimization_ || kModelType_ == superansac::models::Types::Homography ||
        kModelType_ == superansac::models::Types::RigidTransformation ||
        kModelType_ == superansac::models::Types::EssentialMatrix ||
        kModelType_ == superansac::models::Types::FundamentalMatrix)
    ...
    if (kFinalOptimization_) {
      ...
      lsqLocalOptimizer->setSigmaScheduleRange(kSettings_.finalRefinementScheduleStart,
                                               kSettings_.finalRefinementScheduleEnd);
      lsqLocalOptimizer->setWeightedFit(kSettings_.finalRefinementWeighted);
    }
  } else if (kLocalOptimizationType_ ==
             superansac::local_optimization::LocalOptimizationType::NestedRANSAC) {
    // Set the neighborhood graph to the local optimizer
    auto nestedRansacLocalOptimizer =
        dynamic_cast<superansac::local_optimization::NestedRANSACOptimizer*>(localOptimizer_.get());
    ...
    nestedRansacLocalOptimizer->setSampleSizeMultiplier(kLOSettings_.sampleSizeMultiplier);
             ...
             superansac::local_optimization::LocalOptimizationType::IteratedLMEDS) {
    auto iteratedLMEDSLocalOptimizer =
        dynamic_cast<superansac::local_optimization::IteratedLMEDSOptimizer*>(
            localOptimizer_.get());
    iteratedLMEDSLocalOptimizer->setModelType(kModelType_);
  } else if (kLocalOptimizationType_ ==
             superansac::local_optimization::LocalOptimizationType::CrossValidation) {
    auto crossValidationLocalOptimizer =
        dynamic_cast<superansac::local_optimization::CrossValidationOptimizer*>(
            localOptimizer_.get());
    if (kFinalOptimization_ || kModelType_ == superansac::models::Types::Homography ||
        kModelType_ == superansac::models::Types::RigidTransformation ||
        kModelType_ == superansac::models::Types::EssentialMatrix ||
...
std::tuple<Eigen::Matrix3d, Eigen::Vector3d, std::vector<size_t>, double, size_t>
estimateAbsolutePose(
    ...
    const std::vector<double>&
        kBoundingBox_,  // The bounding box dimensions (image width, image height, X, Y, Z)
    const std::vector<double>&
    ...
    superansac::RANSACSettings& settings_)  // The RANSAC settings
{
  ...
  if (settings_.sampler == superansac::samplers::SamplerType::ImportanceSampler &&
    ...
    throw std::invalid_argument(
        "The point probabilities must have the same number of elements as the number of "
        "correspondences when using the ImportanceSampler or the ARSampler.");
  ...
  settings_.inlierThreshold = currentCamera->normalizeThreshold(settings_.inlierThreshold);
  ...
  const superansac::local_optimization::LocalOptimizationType kLocalOptimization =
      settings_.localOptimization;
  const superansac::local_optimization::LocalOptimizationType kFinalOptimization =
      settings_.finalOptimization;
  const superansac::termination::TerminationType kTerminationCriterion =
      settings_.terminationCriterion;

  // Create the solvers and the estimator
  std::unique_ptr<superansac::estimator::AbsolutePoseEstimator> estimator =
      std::unique_ptr<superansac::estimator::AbsolutePoseEstimator>(
          new superansac::estimator::AbsolutePoseEstimator());
  ...
  dynamic_cast<superansac::estimator::solver::PnPBundleAdjustmentSolver*>(
      estimator->getMutableNonMinimalSolver())
      ->setCamera(onlyScaledCamera.get());
  auto& solverOptions = dynamic_cast<superansac::estimator::solver::PnPBundleAdjustmentSolver*>(
                            estimator->getMutableNonMinimalSolver())
                            ->getMutableOptions();
  ...
  if (kSampler == superansac::samplers::SamplerType::PROSAC)
    ...
    auto pNapsacSampler =
        dynamic_cast<superansac::samplers::ProgressiveNAPSACSampler<5>*>(sampler.get());
    pNapsacSampler->setSampleSize(estimator->sampleSize());
    pNapsacSampler->setLayerData({16, 8, 4, 2}, kBoundingBox_);
    ...
    dynamic_cast<superansac::samplers::ImportanceSampler*>(sampler.get())
        ->setProbabilities(kPointProbabilities_);  // Set the probabilities to the sampler
  ...
  if (kScoring == superansac::scoring::ScoringType::
                      MAGSAC)  // Initialize the scoring object if the scoring is MAGSAC
  {
    ...
    solverOptions.loss_type = poselib::BundleOptions::LossType::MAGSACPlusPlus;
                             ...
                             ACRANSAC)  // Initialize the scoring object if the scoring is ACRANSAC
    throw std::invalid_argument(
        "The ACRANSAC scoring is not implemented for the absolute pose estimation.");
  ...
  robustEstimator.setTerminationCriterion(
      terminationCriterion.get());  // Set the termination criterion
  ...
  if (kLocalOptimization != superansac::local_optimization::LocalOptimizationType::None) {
    ...
    initializeLocalOptimizer<5>(kCorrespondences_, localOptimizer, neighborhoodGraph, kNeighborhood,
                                kLocalOptimization, kBoundingBox_, settings_,
                                settings_.localOptimizationSettings,
                                superansac::models::Types::AbsolutePose, false);
  ...
  if (kFinalOptimization != superansac::local_optimization::LocalOptimizationType::None) {
    ...
    initializeLocalOptimizer<5>(kCorrespondences_, finalOptimizer, neighborhoodGraph, kNeighborhood,
                                kFinalOptimization, kBoundingBox_, settings_,
                                settings_.finalOptimizationSettings,
                                superansac::models::Types::AbsolutePose, true);
  ...
  if (robustEstimator.getInliers().size() < estimator->sampleSize())
    return std::make_tuple(Eigen::Matrix3d::Identity(), Eigen::Vector3d::Zero(),
                           std::vector<size_t>(), 0.0, robustEstimator.getIterationNumber());
  ...
  return std::make_tuple(rotation, translation, robustEstimator.getInliers(),
    ...
    superansac::RANSACSettings& settings_)  // The RANSAC settings
{
  ...
  std::unique_ptr<superansac::estimator::RigidTransformationEstimator> estimator =
      std::unique_ptr<superansac::estimator::RigidTransformationEstimator>(
          new superansac::estimator::RigidTransformationEstimator());
  estimator->setMinimalSolver(new superansac::estimator::solver::RigidTransformProscrutesSolver());
  estimator->setNonMinimalSolver(
      new superansac::estimator::solver::RigidTransformProscrutesSolver());
  ...
  if (kSampler == superansac::samplers::SamplerType::PROSAC)
    ...
    auto pNapsacSampler =
        dynamic_cast<superansac::samplers::ProgressiveNAPSACSampler<6>*>(sampler.get());
    pNapsacSampler->setSampleSize(estimator->sampleSize());
    pNapsacSampler->setLayerData({16, 8, 4, 2}, kBoundingBoxSizes_);
  } else if (kSampler == superansac::samplers::SamplerType::NAPSAC) {
    // Initialize the neighborhood graph
                      ...
                      GRID)  // Set the neighborhood structure if the scoring is GRID
  {
    ...
    if (neighborhoodGraph == nullptr)
      ...
      initializeNeighborhood<6>(kCorrespondences_,   // The point correspondences
                                ...
                                kBoundingBoxSizes_,  // Bounding box sizes (x1, y1, z1, x2, y2, z2)
                                settings_);          // The RANSAC settings
    else if (kNeighborhood != superansac::neighborhood::NeighborhoodType::
  ...
  if (kLocalOptimization != superansac::local_optimization::LocalOptimizationType::None) {
    ...
    // Initialize the local optimizer
    initializeLocalOptimizer<6>(kCorrespondences_, localOptimizer, neighborhoodGraph, kNeighborhood,
                                kLocalOptimization, kBoundingBoxSizes_, settings_,
                                settings_.localOptimizationSettings,
                                superansac::models::Types::RigidTransformation, false);
  ...
  if (kFinalOptimization != superansac::local_optimization::LocalOptimizationType::None) {
    ...
    // Initialize the final optimizer
    initializeLocalOptimizer<6>(kCorrespondences_, finalOptimizer, neighborhoodGraph, kNeighborhood,
                                kFinalOptimization, kBoundingBoxSizes_, settings_,
                                settings_.finalOptimizationSettings,
                                superansac::models::Types::RigidTransformation, true);
  ...
  // Check if the model is valid
  if (robustEstimator.getInliers().size() < estimator->sampleSize())
    return std::make_tuple(Eigen::Matrix4d::Identity(), std::vector<size_t>(), 0.0,
                           robustEstimator.getIterationNumber());
  ...
  // Return the best model with the inliers and the score
  return std::make_tuple(rigidTransform, robustEstimator.getInliers(),
                         robustEstimator.getBestScore().getValue(),
                         robustEstimator.getIterationNumber());
}
...
std::tuple<Eigen::Matrix3d, std::vector<size_t>, double, size_t> estimateFundamentalMatrix(
    ...
    const std::vector<double>&
        kPointProbabilities_,  // The probabilities of the points being inliers
    const std::vector<double>&
        kImageSizes_,  // Image sizes (width source, height source, width destination, height destination)
    superansac::RANSACSettings& settings_)  // The RANSAC settings
{
  // Check if the input matrix has the correct dimensions
  ...
  if (kImageSizes_.size() != 4)
    throw std::invalid_argument(
        "The image sizes must have 4 elements (height source, width source, height destination, "
        "width destination).");
  if (settings_.sampler == superansac::samplers::SamplerType::ImportanceSampler &&
      kPointProbabilities_.size() != kCorrespondences_.rows())
    throw std::invalid_argument(
        "The point probabilities must have the same number of elements as the number of "
        "correspondences when using the ImportanceSampler or the ARSampler.");
  if (kPointProbabilities_.size() > 0 && kPointProbabilities_.size() != kCorrespondences_.rows())
    throw std::invalid_argument(
        "The point probabilities must have either the same number of elements as the number of "
        "correspondences or none.");

  // Normalize the point correspondences
  DataMatrix normalizedCorrespondences;
  Eigen::Matrix3d normalizingTransformSource, normalizingTransformDestination;
  normalize2D2DPointCorrespondences(kCorrespondences_, normalizedCorrespondences,
                                    normalizingTransformSource, normalizingTransformDestination);
  ...
  std::unique_ptr<superansac::estimator::FundamentalMatrixEstimator> estimator =
      std::unique_ptr<superansac::estimator::FundamentalMatrixEstimator>(
          new superansac::estimator::FundamentalMatrixEstimator());
  estimator->setOrientationCheck(settings_.fundamentalOrientationCheck);
  estimator->setMinimalSolver(
      new superansac::estimator::solver::FundamentalMatrixSevenPointSolver());
  estimator->setNonMinimalSolver(
      new superansac::estimator::solver::FundamentalMatrixBundleAdjustmentSolver());
  superansac::estimator::solver::FundamentalMatrixBundleAdjustmentSolver* nonminimalSolver =
      dynamic_cast<superansac::estimator::solver::FundamentalMatrixBundleAdjustmentSolver*>(
          estimator->getMutableNonMinimalSolver());
  if (kPointProbabilities_.size() > 0) nonminimalSolver->setWeights(&kPointProbabilities_);
  ...
  if (kSampler == superansac::samplers::SamplerType::PROSAC)
    ...
    auto pNapsacSampler =
        dynamic_cast<superansac::samplers::ProgressiveNAPSACSampler<4>*>(sampler.get());
    pNapsacSampler->setSampleSize(estimator->sampleSize());
  ...
  if (settings_.inlierSelector != superansac::inlier_selector::InlierSelectorType::None) {
    ...
    if (kScoring != superansac::scoring::ScoringType::RANSAC &&
      ...
      throw std::invalid_argument(
          "The space partitioning inlier selector can only be used with RANSAC, MSAC, or MAGSAC++ "
          "scoring.");

    // Check if the neighborhood is grid
    if (kNeighborhood != superansac::neighborhood::NeighborhoodType::Grid)
      throw std::invalid_argument(
          "The space partitioning inlier selector can only be used with grid neighborhood.");
    ...
    // Create the inlier selector
    inlierSelector = superansac::inlier_selector::createInlierSelector(
        superansac::inlier_selector::InlierSelectorType::SpacePartitioningRANSAC);
    // Initialize the inlier selector
    superansac::inlier_selector::SpacePartitioningRANSAC* spacePartitioningRANSAC =
        reinterpret_cast<superansac::inlier_selector::SpacePartitioningRANSAC*>(
            inlierSelector.get());
    spacePartitioningRANSAC->initialize(neighborhoodGraph.get(),
                                        superansac::models::Types::FundamentalMatrix);
    spacePartitioningRANSAC->setNormalizers(
        normalizingTransformSource(0, 0), normalizingTransformSource(0, 2),
        normalizingTransformSource(1, 2), normalizingTransformDestination(0, 0),
        normalizingTransformDestination(0, 2), normalizingTransformDestination(1, 2));
  ...
  if (kLocalOptimization != superansac::local_optimization::LocalOptimizationType::None) {
    ...
    // Initialize the local optimizer
    initializeLocalOptimizer<4>(kCorrespondences_, localOptimizer, neighborhoodGraph, kNeighborhood,
                                kLocalOptimization, kImageSizes_, settings_,
                                settings_.localOptimizationSettings,
                                superansac::models::Types::FundamentalMatrix, false);
  ...
  if (kFinalOptimization != superansac::local_optimization::LocalOptimizationType::None) {
    ...
    // Initialize the final optimizer
    initializeLocalOptimizer<4>(kCorrespondences_, finalOptimizer, neighborhoodGraph, kNeighborhood,
                                kFinalOptimization, kImageSizes_, settings_,
                                settings_.finalOptimizationSettings,
                                superansac::models::Types::FundamentalMatrix, true);
  ...
  // Check if the model is valid
  if (robustEstimator.getInliers().size() < estimator->sampleSize())
    return std::make_tuple(Eigen::Matrix3d::Identity(), std::vector<size_t>(), 0.0,
                           robustEstimator.getIterationNumber());
  ...
  // Transform the estimated fundamental matrix back to the not normalized space
  fundamentalMatrix =
      normalizingTransformDestination.transpose() * fundamentalMatrix * normalizingTransformSource;
  fundamentalMatrix.normalize();

  // Return the best model with the inliers and the score
  return std::make_tuple(fundamentalMatrix, robustEstimator.getInliers(),
                         robustEstimator.getBestScore().getValue(),
                         robustEstimator.getIterationNumber());
}
...
std::tuple<Eigen::Matrix3d, std::vector<size_t>, double, size_t> estimateEssentialMatrix(
    ...
    const Eigen::Matrix3d&
        kIntrinsicsDestination_,  // The intrinsic matrix of the destination camera
    const std::vector<double>&
        kPointProbabilities_,  // The probabilities of the points being inliers
    const std::vector<double>&
        kImageSizes_,  // Image sizes (width source, height source, width destination, height destination)
    superansac::RANSACSettings& settings_)  // The RANSAC settings
{
  ...
  DataMatrix normalizedCorrespondences;
  Eigen::Matrix3d normalizingTransformSource, normalizingTransformDestination;
  normalizePointsByIntrinsics(kCorrespondences_, kIntrinsicsSource_, kIntrinsicsDestination_,
                              normalizedCorrespondences);

  const double kScale = 0.25 * (kIntrinsicsSource_(0, 0) + kIntrinsicsSource_(1, 1) +
                                kIntrinsicsDestination_(0, 0) + kIntrinsicsDestination_(1, 1));
  settings_.inlierThreshold /= kScale;
  ...
  std::unique_ptr<superansac::estimator::EssentialMatrixEstimator> estimator =
      std::unique_ptr<superansac::estimator::EssentialMatrixEstimator>(
          new superansac::estimator::EssentialMatrixEstimator());
  estimator->setMinimalSolver(
      new superansac::estimator::solver::EssentialMatrixFivePointNisterSolver());
  estimator->setNonMinimalSolver(
      new superansac::estimator::solver::EssentialMatrixBundleAdjustmentSolver());
  superansac::estimator::solver::EssentialMatrixBundleAdjustmentSolver* nonminimalSolver =
      dynamic_cast<superansac::estimator::solver::EssentialMatrixBundleAdjustmentSolver*>(
          estimator->getMutableNonMinimalSolver());
  if (kPointProbabilities_.size() > 0) nonminimalSolver->setWeights(&kPointProbabilities_);
  ...
  // Return the best model with the inliers and the score
  return std::make_tuple(essentialMatrix, robustEstimator.getInliers(),
                         robustEstimator.getBestScore().getValue(),
                         robustEstimator.getIterationNumber());
}
    ...
    superansac::RANSACSettings& settings_)  // The RANSAC settings
{
  ...
  std::unique_ptr<superansac::estimator::HomographyEstimator> estimator =
      std::unique_ptr<superansac::estimator::HomographyEstimator>(
          new superansac::estimator::HomographyEstimator());
  ...
  estimator->setBundleRefinement(settings_.homographyBundleRefinement, settings_.inlierThreshold,
                                 settings_.homographyBundleMaxIterations);
  ...
  // Return the best model with the inliers and the score
  return std::make_tuple(robustEstimator.getBestModel().getData(), robustEstimator.getInliers(),
                         robustEstimator.getBestScore().getValue(),
                         robustEstimator.getIterationNumber());
}