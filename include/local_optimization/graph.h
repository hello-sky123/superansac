...
private:
	...
	// Sentinel values stored in the index fields below.
	// They replace the special pointer values of the original
	// (pointer-based) implementation:
	//   NULL pointer        -> INDEX_NONE
	//   TERMINAL ((arc*)1)  -> INDEX_TERMINAL  (node->parent only)
	//   ORPHAN   ((arc*)2)  -> INDEX_ORPHAN    (node->parent only)
	static const int INDEX_NONE     = -1;
	static const int INDEX_TERMINAL = -2;
	static const int INDEX_ORPHAN   = -3;
...
public:
	...
	////////////////////////////////////////////////////////////////////////////////
	// 2. Functions for getting arc ids and for reading graph structure.          //
	//    Arcs are identified by their index in the internal arc array.           //
	////////////////////////////////////////////////////////////////////////////////
...
private:
	...
	struct node
	{
		int			first;		// index of the first outcoming arc (INDEX_NONE if none)

		int			parent;		// index of the node's parent arc
								//   (INDEX_NONE / INDEX_TERMINAL / INDEX_ORPHAN are special values)
		int			next;		// index of the next active node
								//   (or its own index if it is the last node in the list;
								//    INDEX_NONE if the node is not in the list)
		int			TS;			// timestamp showing when DIST was computed
		int			DIST;		// distance to the terminal
		int			is_sink : 1;	// flag showing whether the node is in the source or in the sink tree (if parent!=INDEX_NONE)
	...
	struct arc
	{
		int			head;		// index of the node the arc points to
		int			next;		// index of the next arc with the same originating node (INDEX_NONE if none)
		int			sister;		// index of the reverse arc
	...
	int					node_num;
	int					node_num_max;
	int					arc_num;		// number of arcs added so far (2 per edge)
	int					arc_num_max;
	...
	int					queue_first[2], queue_last[2];	// list of active nodes (node indices, INDEX_NONE if empty)
	...
	void set_orphan_front(int i); // add to the beginning of the list
	void set_orphan_rear(int i);  // add to the end of the list

	void add_to_changed_list(int i);

	void maxflow_init();             // called if reuse_trees == false
	void maxflow_reuse_trees_init(); // called if reuse_trees == true
	void augment(int middle_arc);
	void process_source_orphan(int i);
	void process_sink_orphan(int i);

	void test_consistency(int current_node = -1); // debug function
};
	...
	inline typename Graph<captype,tcaptype,flowtype>::node_id Graph<captype,tcaptype,flowtype>::add_node(int num)
{
	...
	if (node_num + num > node_num_max) reallocate_nodes(num);

	if (num == 1)
	{
		nodes[node_num].first = INDEX_NONE;
		nodes[node_num].tr_cap = 0;
		nodes[node_num].is_marked = 0;
		nodes[node_num].is_in_changed_list = 0;

		return node_num ++;
	}
	else
	{
		memset(nodes + node_num, 0, num*sizeof(node));
		for (int k = node_num; k < node_num + num; k ++)
		{
			nodes[k].first = INDEX_NONE;
			nodes[k].parent = INDEX_NONE;
			nodes[k].next = INDEX_NONE;
		}
	...
	inline void Graph<captype,tcaptype,flowtype>::add_edge(node_id _i, node_id _j, captype cap, captype rev_cap)
{
	...
	if (arc_num == arc_num_max) reallocate_arcs();
	...
	arcs[a].sister = a_rev;
	arcs[a_rev].sister = a;
	arcs[a].next = nodes[_i].first;
	nodes[_i].first = a;
	arcs[a_rev].next = nodes[_j].first;
	nodes[_j].first = a_rev;
	arcs[a].head = _j;
	arcs[a_rev].head = _i;
	arcs[a].r_cap = cap;
	arcs[a_rev].r_cap = rev_cap;
}

template <typename captype, typename tcaptype, typename flowtype>
	inline typename Graph<captype,tcaptype,flowtype>::arc_id Graph<captype,tcaptype,flowtype>::get_first_arc()
{
	return 0;
}

template <typename captype, typename tcaptype, typename flowtype>
	inline typename Graph<captype,tcaptype,flowtype>::arc_id Graph<captype,tcaptype,flowtype>::get_next_arc(arc_id a)
{
	return a + 1;
}

template <typename captype, typename tcaptype, typename flowtype>
	inline void Graph<captype,tcaptype,flowtype>::get_arc_ends(arc_id a, node_id& i, node_id& j)
{
	assert(a >= 0 && a < arc_num);
	i = arcs[arcs[a].sister].head;
	j = arcs[a].head;
}
...
{
	node* i = nodes + _i;
	if (i->next == INDEX_NONE)
	{
		/* it's not in the list yet */
		if (queue_last[1] != INDEX_NONE) nodes[queue_last[1]].next = _i;
		else                             queue_first[1]            = _i;
		queue_last[1] = _i;
		i -> next = _i;
	}
	i->is_marked = 1;
