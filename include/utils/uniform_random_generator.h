...
inline uint64_t uniform_u64_closed(Xoshiro256ss& rng, uint64_t lo, uint64_t hi) {
  ...
  if (l < n) {
    ...
    while (l < t) {
      m = (__uint128_t)rng() * (__uint128_t)n;
      l = (uint64_t)m;
    }
  }
...
class UniformRandomGenerator {
 public:
  ...
  // Legacy API: draw one
  FORCE_INLINE _Type getRandomNumber() { return uniform_closed<_Type>(engine_, min_, max_); }
  ...
  FORCE_INLINE void generateUniqueRandomSet(_Type* sample, const _Type& k, const _Type& maxv) {
  ...
  FORCE_INLINE void generateUniqueRandomSet(_Type* sample, const _Type k, const _Type maxv,
    ...
    size_t w = 0;
    for (auto v : tmp)
      if (v != toSkip && w < (size_t)k) sample[w++] = v;
    while (w < (size_t)k) {
      _Type x;
      do {
        x = uniform_closed<_Type>(engine_, 0, maxv);
